...
async def get_available_balance() -> float:
    """AVAILABLE (kullanilabilir) USDT — acik pozlarin margin'i haric.

    /fapi/v2/balance response'ta 'availableBalance' field'i. Yeni LIMIT emri
    icin bu miktar kritik: total_wallet_balance overestimate eder cunku
    acik pozlarin locked margin'ini de icerir → -2019 Margin insufficient.
    """
    client = await get_client()
    params = _sign({})
    resp = await client.get("/fapi/v2/balance", params=params)
